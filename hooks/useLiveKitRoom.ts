...
interface UseLiveKitRoomOptions {
  onMessageComplete?: (role: "user" | "assistant", text: string) => void;
}
...
export const useLiveKitRoom = (
  ...
  const handleTrackSubscribed = useCallback(
    (
      ...
      if (track.kind === Track.Kind.Audio) {
        ...
        audioElement.muted = true; // Muted until video ready for lip sync
        track.attach(audioElement);
        audioElementRef.current = audioElement;
        console.log("🎵 Audio track attached (muted until video ready)");
      ...
      if (track.kind === Track.Kind.Video) {
        ...
        const attachVideo = () => {
          ...
          if (!videoEl) {
            console.warn("⚠️ Video element not found - audio-only mode");
  ...
  const handleTranscriptionReceived = useCallback(
    (
      ...
      console.log(
        `[useLiveKitRoom] 📝 Transcription from ${isAgent ? "AGENT" : "USER"} (${participant?.identity}):`,
        segments.map((s) => ({ text: s.text, final: s.final })),
      );

      const setter = isAgent ? setAgentSegments : setUserSegments;
      ...
      setter((prevMap) => {
        ...
        segments.forEach((seg) => {
          ...
          // Save to Supabase when a segment transitions to final
          if (seg.final && (!oldSeg || !oldSeg.final)) {
            if (
              ...
              options.onMessageComplete(role, seg.text.trim());
            }
          }
          ...
          async (reader, participantInfo) => {
            ...
            if (isAgent) {
              ...
              for await (const chunk of reader) {
                fullText = chunk; // TextStreamReader yields cumulative text
                setAgentSegments(
                  new Map([
              ...
              if (fullText.trim()) {
                ...
                if (options?.onMessageComplete) {
                  options.onMessageComplete("assistant", fullText.trim());
                }
              }
            }
          },
