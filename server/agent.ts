...
const {
  GOOGLE_API_KEY: SERVER_GOOGLE_API_KEY, // Gemini Live API para LLM
  VITE_GEMINI_API_KEY,
  DEEPGRAM_API_KEY, // Deepgram para STT (transcrição)
  BEY_API_KEY,
  BEY_AVATAR_ID,
  LIVEKIT_URL: SERVER_LIVEKIT_URL,
  LIVEKIT_API_KEY: SERVER_LIVEKIT_API_KEY,
  LIVEKIT_API_SECRET: SERVER_LIVEKIT_API_SECRET,
  VITE_LIVEKIT_URL,
...
const GOOGLE_API_KEY = SERVER_GOOGLE_API_KEY || VITE_GEMINI_API_KEY;
const LIVEKIT_URL = SERVER_LIVEKIT_URL || VITE_LIVEKIT_URL;
const LIVEKIT_API_KEY = SERVER_LIVEKIT_API_KEY || VITE_LIVEKIT_API_KEY;
const LIVEKIT_API_SECRET = SERVER_LIVEKIT_API_SECRET || VITE_LIVEKIT_API_SECRET;
...
if (!BEY_API_KEY) console.warn("⚠️ BEY_API_KEY missing - avatar video will not start");
if (!BEY_AVATAR_ID) console.warn("⚠️ BEY_AVATAR_ID missing - avatar video will not start");
...
process.on("uncaughtException", (err: Error) => {
  console.error("[AGENT] uncaughtException:", err?.stack ?? err);
});
  ...
  entry: async (ctx) => {
    ...
    try {
      ...
      voiceSession.on("conversation_item_added", async (ev: any) => {
        ...
        logWithMetrics(`Saving ${role} message`, { contentLength: content.length });
