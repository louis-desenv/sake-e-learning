    ...
    <div className="w-full h-full flex items-center justify-center bg-gray-50 p-4">
      ...
      <div className="relative w-full max-w-3xl h-[82vh] bg-black rounded-2xl overflow-hidden shadow-xl flex flex-col">
            ...
            <>
              ...
              <div className="absolute top-0 left-0 right-0 z-40 p-8 pt-16 text-center">
                <h1 className="text-4xl font-bold text-white mb-2 drop-shadow-2xl">
                  {t('voiceChat.justSpeak')}
                </h1>
                <p className="text-white/80 text-lg">
                  {t('voiceChat.tapBelow')}
                </p>
              </div>
              ...
              <div className="absolute top-4 right-4 flex items-center space-x-2 z-10">
                {participants.slice(0, 4).map((p, idx) => (
                  <div
                    ...
                    title={p.identity || t('voiceChat.participant')}
                  >
              ...
              <div className="absolute left-1/2 -translate-x-1/2 bottom-28 max-w-[86%] md:max-w-[56%] bg-white/8 backdrop-blur border border-white/10 text-white text-sm px-4 py-3 rounded-2xl z-10">
                <div className="flex items-center justify-between">
                  <div className="text-xs text-white/90 truncate">
                    <strong className="text-white">{t('voiceChat.agent')}:</strong> {agentTranscript || '...'}
                  </div>
                  <div className="text-xs text-white/80 ml-3">{isAgentSpeaking ? t('chat.speaking') : ''}</div>
                </div>
                <div className="text-xs text-white/70 mt-2 truncate">
                  <strong className="text-white">{t('voiceChat.you')}:</strong> {userTranscript || '...'}
                </div>
              </div>
              ...
              <div className="absolute left-1/2 -translate-x-1/2 bottom-6 w-[92%] md:w-2/3 bg-white/6 backdrop-blur border border-white/8 rounded-full px-4 py-3 flex items-center justify-between shadow-2xl z-20">
                <div className="flex items-center space-x-4">
                  <button
                    ...
                    title={t('liveKitPage.toggleMic') || "Toggle Microphone"}
                  >
                    ...
                    <span className="text-[11px] mt-1 text-white/80">{t('voiceChat.mic')}</span>
                  </button>

                  <div className="hidden md:flex flex-col items-start ml-2">
                    <div className="text-xs font-semibold text-white">{user.name || t('voiceChat.you')}</div>
                    <div className="text-[11px] text-white/70">{isConnected ? t('voiceChat.inCall') : t('voiceChat.idle')}</div>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-4">
                  ...
                  >
                    ...
                    <span className="text-sm">{t('voiceChat.endCall')}</span>
                  </button>
                </div>
              </div>
            </>
        ...
        <div className="h-14 bg-black/60 text-white flex items-center justify-between px-4">
          <div className="text-sm">{isConnected ? t('voiceChat.connectedParticipants', { count: participants.length }) : t('voiceChat.notConnected')}</div>
          <div className="flex items-center space-x-3">
            <div className="text-xs opacity-80">{connectionState}</div>
            <div className="text-xs opacity-90">{t('voiceChat.audioOnly')}</div>
          </div>
        </div>
      </div>
    </div>
